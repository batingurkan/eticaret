...
public class UserOrderPendingApproval:ViewComponent
{
    private readonly DataContext _dataContext;
    private readonly IMapper _mapper;

    public UserOrderPendingApproval(DataContext dataContext, IMapper mapper)
    {
    ...
    public async Task<IViewComponentResult> InvokeAsync()
    {
        ...
        var list = await _dataContext.Order.Include(p => p.User).Include(p => p.OrderDetail).Include(p => p.OrderStatus)
            .Include(p => p.Cargo).Where(x => x.OrderStatus.Id == 1 && x.UserId==userId).ToListAsync();
